"""Production ASGI entrypoint for the ola-guardrails sidecar.

Wires the real NeMo jailbreak-detection engine into the FastAPI app and serves it
over mTLS using the shared ``ola-gateway-shared`` helpers. The listener requires a
client certificate; the application layer then enforces the pinned peer-CN allowlist
via :func:`ola_gateway_shared.transport.resolve_caller_cn` and
...
def build_app(engine=None):
    """Construct the production FastAPI app with the real NeMo engine."""
    if engine is None:
...
def run() -> None:
    ...
    assert uvicorn.__version__ == "0.49.0", (
        "ola-guardrails pins uvicorn 0.49.0: the mTLS peer-CN injection "
